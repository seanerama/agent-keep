# Operator runbook — provision a GCP host, then deploy a chassis onto it
...
> onto it — exactly as they do for the AWS module, the NSAF dev server, or any
> bring-your-own host. Same uniform interface as `provision/aws/`.
>
> **This creates a REAL, BILLABLE GCE instance** (an `e2-small` by default) in
> project **agent-keep-kn6r6i**. Keep it short-lived: provision → bootstrap →
> deploy → smoke → **`terraform destroy`**. `destroy` is the off switch; run it as
> soon as you're done.

## What this is / is NOT

- The Terraform module (`provision/gcp/`) makes a VM exist, SSH-reachable, with a
  firewall that allows SSH from **your IP only** and open egress (GCP default).
  That is its whole job (ADR 0009).
...
- **gcloud + Application Default Credentials.** Terraform's google provider reads
  ADC — set them up (the module carries NO credentials):
  ```sh
  gcloud auth login                              # your Google account
  gcloud auth application-default login          # writes ADC Terraform reads
  gcloud config set project agent-keep-kn6r6i    # optional; module defaults to it
  ```
  Confirm: `gcloud auth application-default print-access-token >/dev/null && echo OK`.
  The project **agent-keep-kn6r6i** is already created, billing-linked, and has the
...
- Optionally `instance_size` (default `e2-small`, x86_64 only — ADR 0009, NOT a
  `t2a`/arm type), `region` (default `us-central1`), `zone` (default
  `us-central1-a`), `project` (default `agent-keep-kn6r6i`), `name` (default
  `agent-keep`).
...
```sh
TARGET="$(terraform output -raw ssh_target)"   # e.g. ubuntu@34.x.x.x
echo "$TARGET"
```
...
On a brand-new GCE instance the SSH user is `ubuntu` (the metadata key we set).
...
- The `ubuntu` user has passwordless sudo on Ubuntu images, so the privileged
  install steps run without a password prompt. On success it prints `HOST READY`.
  If Docker was just installed, the script adds `ubuntu` to the `docker` group —
  reconnect once so `docker info` works without sudo.
...
```sh
cd provision/gcp
terraform destroy          # review, type `yes` — tears down the instance + firewall
```

Confirm nothing lingers billing you (the GCE instance is the cost). `destroy`
...
target was **provisioned via `provision/gcp/` (`terraform apply`)**, the project +
region/zone, and that the ssh target comes from `terraform output -raw ssh_target`
— **not** the raw IP or any secret. Because GCP targets are ephemeral, note the
target is only valid between `apply` and `destroy`.

## Cost & safety recap

- Real, billable `e2-small` + a 20 GB pd-balanced boot disk in project
  **agent-keep-kn6r6i** — pennies per hour, but **destroy it** when done.
...
- No secrets in git: `terraform.tfvars` and `*.tfstate*` are gitignored; only the
  `.example` is committed. Cloud auth comes from ADC on your workstation, never in
  the module. The provider key (if any) is injected on-host at deploy by
  `deploy-agent.sh`/`deploy.sh`, never here.
