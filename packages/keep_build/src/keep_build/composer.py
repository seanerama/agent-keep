...
#: python:3.12-slim && docker inspect --format='{{index .RepoDigests 0}}'
#: python:3.12-slim`, then replace the digest. Shared by every bake path
#: (agent image here, egress-proxy image in keep_build.egress_proxy).
BASE_IMAGE = (
...
# Base image pinned by digest (carried from the Foundry, SUPPLY-01): see
# keep_build.composer.BASE_IMAGE for the update procedure.
FROM {BASE_IMAGE}
